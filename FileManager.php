...
class FileManager extends Model
{
    ...
    public function uploadVideo($to, $file, $name = '')
    {
        try {
            $extension = pathinfo($file->getClientOriginalName(), PATHINFO_EXTENSION);
            $mime_type = mime_content_type($file->getPathname());
            if (!in_array($mime_type, allowMimesVideos()) || !in_array($extension, allowExtensionsVideos())) {
            ...
            $store->file_name = $file_name;
            $store->save();
            return ['status' => true, 'file' => $store, 'message' => "Video Save Successfully"];
        } catch (\Exception $exception) {
            return ['status' => false, 'file' => [], 'message' => $exception->getMessage()];
        }
    }
